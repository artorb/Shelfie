...
const DropdownMenuToggleDarkmode = ({key}: Props) => {
    const {dark} = useDark();
            ...
            <Menu.Item key={key}>
                <div className={`flex h-16 items-center justify-center`}>
                    <label htmlFor={'darkMode'}
                           className={`text-white text-md flex w-full justify-between mx-4 items-center`}>Darkmode
                        <Switch
                            checked={darkmode}
                            onChange={() => handleChange()}
                            className={`${darkmode ? 'dark:bg-primary_dark-300' : 'bg-primary-300'}
...
export default DropdownMenuToggleDarkmode;